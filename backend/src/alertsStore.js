...
class AlertsStore {
  ...
  create({ ticker, condition, threshold }) {
    if (!ticker || typeof ticker !== 'string') {
      return { ok: false, error: ERR_TICKER_REQUIRED };
    }
    if (!VALID_CONDITIONS.has(condition)) {
      return { ok: false, error: ERR_INVALID_CONDITION };
    }
    const thresh = Number(threshold);
    if (isNaN(thresh) || thresh <= 0) {
      return { ok: false, error: ERR_INVALID_THRESHOLD };
    }
  ...
  evaluateAll(ticker, price, onTriggered) {
    for (const alert of this._alerts.values()) {
      ...
      const crossed =
        (alert.condition === CONDITION_ABOVE && price >= alert.threshold) ||
        (alert.condition === CONDITION_BELOW && price <= alert.threshold);
